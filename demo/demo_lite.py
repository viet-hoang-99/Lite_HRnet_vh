...
# from AID_augmentation import cutout, haS, gridMask
...
def get_pose_estimation_prediction(pose_model, image, center, scale):
    ...
    # random_aug_occlusion(model_input)
    # cutout(model_input)
    # haS(model_input)
    # gridMask(model_input)
...
def parse_args():
    ...
    # general
    parser.add_argument('--cfg', type=str, default='experiments/coco/lite_hrnet/lite_hrnet.yaml')
...
def main():
    ...
    total_params = sum(param.numel() for param in pose_model.parameters())
    print(total_params)
    ...
    a=torch.load(cfg.TEST.MODEL_FILE)
    # print(len(a))
    # count=0
    # for d in a:
    #     b=a.get(d)
    #     print(d)
    #     print(b.cpu().numpy().reshape(-1).shape)
    #     count=count+b.cpu().numpy().reshape(-1).shape[0]
